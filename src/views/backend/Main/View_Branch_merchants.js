...
const View_Branch_merchants = () => {
  ...
  const isallowedBranchs = isAllowedBarances();
  ...
  useEffect(() => {
    const fetchMerchants = async () => {
      try {
        const response = await axios.get("http://10.1.85.10:8089/api/getqruser");
  ...
  const filteredBranchMerchant = merchants.filter((item) => {
    if (!isallowedBranchs) {
      return item.branch_id === user.branch_id; // Return matching items
    } else {
      return true; // Return all items when isallowedBranchs is false
    }
  });
        ...
        <>
          <div className="table-container">
            <Table striped bordered hover responsive className="custom-table">
              ...
              <tbody>
                {filteredBranchMerchant.map((merchant, index) => (
                  <tr key={merchant.id || index}>
                    ...
                    <td className="text-center">
                      {merchant.total_entries || 0}
                    </td>
                    <td className="text-center">{merchant.with_amount || 0}</td>
                    <td className="text-center">
                      {merchant.with_out_amount || 0}
                    </td>
                    <td className="text-center">
                      {merchant.total_amount || 0}
                    </td>
...
export default View_Branch_merchants;
